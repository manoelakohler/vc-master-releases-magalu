...
def central():
    ...
    cliente.obter(cfg.fonte.base_url)
    resposta = cliente.obter(cfg.fonte.url_central, referer=cfg.fonte.base_url)
    return cfg, resposta.conteudo.decode("utf-8", "replace")
...
def test_itr_nao_vira_release(central):
    ...
    documentos = descobrir_documentos(html, base_url=cfg.fonte.base_url)
    for d in documentos:
        if "itr" in d.titulo.lower():
            assert d.tipo is not TipoDocumento.RELEASE_RESULTADOS


def test_relata_paginacao(central):
    _, html = central
    print(f"\npaginação por postback detectada: {tem_paginacao(html)}")
